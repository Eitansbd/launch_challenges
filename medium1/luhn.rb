...
class Luhn
  ...
  def self.create(num)
    digits = num.digits.unshift(0)
    digit_to_add = -(self.checksum(digits)) % 10
    num * 10 + digit_to_add
  end

  def checksum
    self.class.checksum(@digits)
  end

  def self.checksum(digits)
    self.addends(digits).sum
  end

  def addends
    self.class.addends(@digits).reverse
  end

  def self.addends(digits)
   digits.map.with_index do |val, index|
      index.odd? ? double_and_subratract(val) : val
    end
  end
  
  def self.double_and_subratract(value)
    (value * 2 >= 10) ? value * 2 - 9 : (value * 2)
  end
end
